...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      // The fetch call now uses the updated BACKEND_URL which comes from NEXT_PUBLIC_API_URL
  ...
  return (
    ...
    >
      ...
      {success && (
        <div style={{ color: "#090", margin: "14px 0 0" }}>{success}</div>
      )}
      {error && (
      ...
      <div style={{ marginTop: 32, fontSize: 13, color: "#666" }}>
        <b>Extra features:</b> Prompt enrichment (Wikipedia context), video
        streaming, cache, regenerate.
      </div>
    </div>
  );
}
